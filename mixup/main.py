...
def train(train_loader, model, optimizer, epoch, args, log):
    ...
    print_log('  **Train** Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 {error1:.3f}'
...
def validate(val_loader, model, log):
    ...
    print_log('  **Test** Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 {error1:.3f} Loss: {losses.avg:.3f} '
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print_log("=> loaded checkpoint '{}' accuracy={} (epoch {})"
                      .format(args.resume, best_acc, checkpoint['epoch']), log)
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print_log('\n==>>{:s} [Epoch={:03d}/{:03d}] {:s} [learning_rate={:6.4f}]'
                  ...
                  .format(recorder.max_accuracy(False), 100 - recorder.max_accuracy(False)), log)
